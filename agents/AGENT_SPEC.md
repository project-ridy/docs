...
2. **관련 docs 문서 로드** — API 스펙, DB 스키마, 디자인 시스템을 읽고 구현 기준 확립
3. **스킬 로드** — 담당 레포에 맞는 스킬을 로드 (프론트/백 각각)
4. **브랜치 생성** — `feat/<이슈번호>-<설명>` 형식으로 브랜치 생성
5. **TDD 사이클** — 🔴 Red(테스트 작성) → 🟢 Green(최소 구현) → 🔵 Refactor(정리)
6. **PR 생성** — 이슈 연결, 체크리스트 확인, 커밋 메시지 규칙 준수
7. **이슈 Done 처리** — PR 머지 후 Project에서 이슈를 Done으로 변경
...
2. **디자인 문서 로드** — DESIGN_SYSTEM.md, WIREFRAMES.md, SCREENS.md 확인
3. **스킬 로드** — UX/접근성 관련 스킬 로드
4. **브랜치 생성** — `design/<이슈번호>-<설명>` 형식
5. **컴포넌트 구현** — shadcn/ui 기반, 디자인 토큰 준수, 반응형 + 접근성
6. **PR 생성** — 스크린샷/시각적 검증 포함
7. **이슈 Done 처리** — PR 머지 후 Project에서 이슈를 Done으로 변경
