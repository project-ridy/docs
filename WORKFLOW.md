...
- A/E/X 케이스 분석
  ...
  - X: 엣지 케이스
- 구현/테스트 케이스 등록표
  - 기획서의 모든 기능 코드 항목을 케이스 ID로 등록한다.
  - 각 케이스마다 대상 파일, 구현 함수/컴포넌트/Resolver/Service, 대응 테스트 파일, 테스트 이름을 명시한다.
  - 기능 케이스는 대응 테스트 케이스 없이 완료할 수 없다.
- 수정/생성할 파일 경로
- TDD 순서
- 실행할 검증 명령
- 완료 조건
...
- 계획서 순서대로 진행한다.
- 계획서의 구현/테스트 케이스 등록표에 있는 모든 케이스를 구현 대상과 테스트 대상으로 처리한다.
- 새 기능 코드, Resolver, Service, hook, component, page, GraphQL operation은 반드시 케이스 ID와 대응 테스트를 가진다.
- 테스트를 먼저 작성한다.
- 실패를 확인한 뒤 최소 구현한다.
...
- 관련 계획서 경로
- 구현/테스트 케이스 확인표
  - 기획서의 케이스 ID별 구현 파일, 테스트 파일, 테스트명, 확인 결과를 모두 표시한다.
  - 미구현/미검증 케이스는 PR에서 완료로 표시할 수 없고, 범위 제외 또는 BLOCKED 사유를 적는다.
- 검증 결과
- 범위 제외 또는 후속 작업
