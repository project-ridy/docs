...
- `SocialOAuthVerifier` 추상화 및 Kakao/Google/Apple provider verifier
- 휴대폰 SMS 인증 코드 발송/검증 API
- `PhoneVerificationCode` 저장 모델 및 마이그레이션
- `AuthResolver` GraphQL mutation/query 연결
- `AuthGuard` GraphQL context 인증 보호 기반
- `AuthService`/`PhoneVerificationService` TDD 유닛 테스트
