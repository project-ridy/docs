...
Ridy는 **회사 이메일 도메인 기반 폐쇄형 동네→직장 카풀 서비스**로, 외부 API는 **GraphQL Gateway + schema-first** 아키텍처를 사용한다.
...
- **같은 회사 도메인만 매칭**: `nearbyCommuteOffers`/`searchRides` 시 자동으로 같은 `companyId`/`domain` 필터
- **동네→직장 방향 제한**: MVP 매칭은 차주가 등록한 동네 출발 위치에서 회사 근무지로 향하는 출근 카풀만 허용
- **위치 개인정보 보호**: 승인 전에는 정확한 집 주소를 노출하지 않고 표시용 동네명/대략 위치만 반환
...
| 매칭 | [MATCHING.md](./MATCHING.md) | 같은 회사 이메일 도메인 구성원 간 동네→직장 카풀 등록/지도 탐색/요청/수락 | `Ride`, `RideRequest`, `NearbyCommuteOffersInput`, `Workplace` |
