...
폐쇄형 카풀의 기본 단위. 모든 사용자와 카풀은 회사 및 회사 이메일 도메인에 귀속된다.

| 속성 | 설명 |
|---|---|
| `id` | 고유 식별자 |
| `name` | 회사명 |
| `domain` | 회사 이메일 도메인 (예: `acme.co.kr`) |
| `inviteCode` | 가입용 코드 (고유, 변경 가능) |
| `adminId` | 도메인 운영자 User ID |
| `maxMembers` | 최대 사원 수 |
...
- 매칭 서비스는 항상 `WHERE companyId = :currentUser.companyId` 조건으로 검색하고, 가입 시 검증된 `Company.domain` 경계를 신뢰한다.
- 다른 회사 이메일 도메인 사용자는 검색 결과에 노출되지 않는다.
- 카풀 방(Ride)도 회사/도메인에 귀속되어, 크로스 컴퍼니 접근이 원천 차단된다.
...
| 기술 | 용도 |
|---|---|
| Next.js 16 (App Router) | 웹 서비스 (사원 앱 + 도메인 운영 화면) |
| shadcn/ui | UI 컴포넌트 라이브러리 |
| TypeScript | 타입 안전성 |
| Tailwind CSS | 스타일링 |
| TanStack Query | 서버 상태 관리 |
| Socket.IO Client | 실시간 채팅 |
| Recharts | 운영/후속 관리자 대시보드 차트 |
...
## 도메인 운영 화면 / 후속 관리자 대시보드

도메인 운영자 또는 후속 관리자(`UserRole.ADMIN`)만 접근 가능한 전용 화면. `/admin` 경로에 위치한다. MVP에서 필요한 최소 범위는 가입 코드와 구성원 현황이며, ESG 리포트·계약/플랜 관리는 후속 B2B 확장 전까지 BLOCKED다.

### 기능 구성

| 기능 | 설명 |
|---|---|
| **가입 코드 관리** | 신규 코드 생성, 기존 코드 폐기/재발급, 사용 이력 조회 |
| **구성원 관리** | 가입 구성원 목록, 회사 이메일 도메인, 활성/비활성 상태 |
| **이용 통계** | 일/주/월 단위 카풀 이용 건수, 참여율, 노선 히트맵 |
| **ESG 리포트** | 후속 B2B 확장. 탄소 절감량(CO₂), 총 주행 거리 절감, 참여 인원 추이 — 구현 전 BLOCKED |
...
- 운영자/관리자는 후속 대시보드에서 월/분기 단위 리포트를 조회할 수 있다. PDF 다운로드는 별도 스펙 전까지 BLOCKED다.
...
| **운영/관리자** | `@adminOnly` 디렉티브로 운영자/관리자 권한 검증 |
| **GraphQL Gateway** | JWT 검증, query complexity 제한, rate limit, persisted query 적용 |
| **가입 코드** | 단일 사용(1회용) 또는 다중 사용(기간 만료) 설정 가능 |
...
## 마이그레이션 노트 (오픈 → 회사 이메일 도메인 폐쇄형)

상세 DB 컬럼/제약/인덱스와 expand → backfill → validate → contract 마이그레이션 절차는 [`DATABASE.md`](./DATABASE.md)의 “인덱스 전략”과 “마이그레이션 전략”을 기준으로 한다.
...
| 리포트 | 없음 | 개인/도메인 친환경 임팩트. 관리자 ESG 리포트는 후속 B2B |
| 프론트엔드 | 사원 앱 단일 | **사원 앱 + 도메인 운영 화면** (분리 라우트) |
