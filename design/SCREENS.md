...
| S04 | 홈 (탑승자) | / | 내 동네 주변 회사행 카풀 지도 탐색 |
| S05 | 홈 (차주) | /driver | 동네 출발 위치 등록, 요청 관리 |
| S06 | 매칭 검색 결과 | /matchings | 지도 + 카풀 제공자 리스트 |
...
### S04 홈 (탑승자)
- **지도 탐색**: 내 동네/현재 위치 중심으로 주변 회사행 카풀 marker 표시
...
| 지도 | 내 위치/동네 중심 지도, 주변 회사행 카풀 marker | 승인 전 marker는 대략 위치와 `pickupLabel`만 표시 |
| 필터 | 출발 시간대, 회사 근무지, 반경 | 기본 반경 3km, 최대 10km |
...
| 영역 | 구성 요소 | 상태/동작 |
|---|---|---|
| 지도 | 주변 카풀 marker, cluster | marker 선택 시 bottom sheet 카드 표시 |
| 검색 요약 | 선택 동네→회사 근무지, 날짜/시간 | 수정 CTA 제공 |
| 필터 | 시간대, 잔여 좌석, 근무지 | Mobile은 bottom sheet, Desktop은 좌측 패널 |
| 결과 리스트 | 카풀 제공자 카드 | 카드 클릭 시 S07 이동, CTA는 탑승 요청 |
...
| 경로 상세 | 표시용 동네명, 회사 근무지, 예상 시간, 지도 placeholder | 승인 전 정확한 집 주소 비노출 |
