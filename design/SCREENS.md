...
| S02 | 로그인/가입 | /login | 가입 코드, 회사 이메일 검증, 소셜 로그인 |
...
- **이용 방법**: 가입 코드 → 회사 이메일 검증 → OAuth 가입 → 매칭 → 탑승
...
| 이용 방법 | 가입 코드 → 회사 이메일 검증 → OAuth 가입 → 매칭 → 탑승 | 단계 번호와 아이콘 표시 |
...
| 회사 이메일 | 회사 이메일 입력 필드, 도메인 검증 메시지 | 가입 코드가 가리키는 `Company.domain`과 일치해야 다음 단계 가능 |
| 소셜 로그인 | 카카오, 구글 버튼 | 가입 코드와 회사 이메일 검증 전 disabled |
| 안내 | “가입 코드와 회사 이메일로만 가입 가능” 설명 | 회사 이메일 도메인 기반 폐쇄형 서비스 원칙 명시 |

상태: `empty`, `validatingInviteCode`, `invalidInviteCode`, `validatingCompanyEmail`, `invalidCompanyEmailDomain`, `socialLoginPending`, `loginFailed`.
