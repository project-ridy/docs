...
| 영역 | 구성 요소 | 상태/동작 |
|---|---|---|
| 상단 액션 | 왼쪽 메뉴 버튼, 오른쪽 프로필 버튼 | 하단바를 표시하지 않습니다. 메뉴에는 이전 탑승 기록(`/payments`) 진입점을 포함합니다. 프로필 버튼은 `/profile`로 이동합니다. |
| 상단 안내 | “집 주변 회사행 카풀” 제목, 현재 동네/위치 사용 CTA, 1km/2km/5km 반경 선택 | 검색 입력을 제공하지 않습니다. 위치 CTA는 권한 요청 또는 동네 fallback만 담당합니다. 기본 반경은 5km입니다. |
| 지도 | 집/동네 또는 현재 위치 중심 지도, 선택 반경 테두리, 주변 회사행 카풀 marker | 승인 전 marker는 대략 위치와 `pickupLabel`만 표시. 결과는 선택 반경 이내로 제한 |
