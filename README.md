...
**함께 타는 출퇴근, Ridy** — 회사 이메일 도메인 기반 폐쇄형 카풀 매칭 서비스의 설계 문서 저장소

같은 회사 이메일 도메인 구성원끼리만 출퇴근 방향이 맞으면 카풀할 수 있도록 연결합니다. 가입 코드 입력, 회사 이메일 인증코드 검증, 비밀번호 설정으로 외부인을 차단하고, 사내 검증된 환경에서 안전한 카풀을 제공합니다.
...
│   ├── AUTH.md            # 인증/인가 API (가입 코드와 회사 이메일 검증 포함)
│   ├── MATCHING.md        # 매칭 API (같은 회사 이메일 도메인 구성원만)
│   ├── CHAT.md            # 채팅 API
